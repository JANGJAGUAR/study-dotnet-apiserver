...
{
    public partial class OmokClient
    {
        ...
        #region omok UI
        void OmokBoard_Paint(object sender, PaintEventArgs e)
        {
        ...
        {
            Graphics g = OmokBoard.CreateGraphics();
        ...
        void 다음돌표시()        // 화면 하단에 다음에 둘 돌의 색을 표시
        {
            ...
            {
                str = "흑돌 차례";
                g.DrawString(str, 글꼴, 검은색, 시작위치+돌표시x위치, 돌표시y위치);
                g.FillEllipse(검은색, 시작위치 + 돌표시x위치 - 40, 돌표시y위치-4, 돌크기, 돌크기);
            }
            ...
            else                 // 다음 돌 표시(흰 돌)
            {
                str = "백돌 차례";
                g.DrawString(str, 글꼴, 검은색, 시작위치+돌표시x위치, 돌표시y위치);
                g.FillEllipse(흰색, 시작위치 + 돌표시x위치 - 40, 돌표시y위치-4, 돌크기, 돌크기);
            }
        ...
        private void OmokBoard_MouseMove(object sender, MouseEventArgs e)     // 현재 차례의 돌 잔상 구현 (마우스 움직일때)
        {
            int x, y;
            ...
            else if (바둑판[x, y] == (int)돌종류.없음 && !게임종료 && (전x좌표 != x || 전y좌표 != y))             // 바둑판 해당 좌표에 아무것도 없고, 좌표가 변경되면
            {
                ...
                OmokBoard.Invalidate(앞에찍은돌을지우기위한구역);               // 먼저 그린 잔상을 지우고 새로운 잔상을 그린다.
